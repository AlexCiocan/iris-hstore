...
public class LeadStorageRepositoryImpl implements LeadStorageRepository {
    ...
    public List<Lead> getLead(Long orderId, Long partnerId) {
        ...
        if (partnerId != null) {
            predicates.add(builder.and(builder.equal(leadRoot.get("partnerId"), partnerId)));
        }

        query.where(predicates.toArray(new Predicate[predicates.size()]));
        List<Lead> leads = getEntityManager().createQuery(query).getResultList();
        return leads;
    }

    @Override
    public LeadData getLeadData(Long leadId) {
        Lead lead = getEntityManager().createNamedQuery("lead.byLeadId", Lead.class).setParameter("leadId", leadId)
                .getSingleResult();
    ...
    public Long saveLeadData(LeadData leadData) {
        save(leadData);
        //TODO refactor this -> this seems to force an additional load operation for the not owning side
